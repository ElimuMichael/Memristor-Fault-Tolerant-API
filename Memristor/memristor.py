...
#         w_b_C_T = np.hstack((weights, biases))
        ...
        if fix_used == 'none':            
            ...
            # print("bits Prop\n{}".format(type(bits[0])))
            L_weights['weights'] = xbar_output(bits[0],w_inp, w_dim)
            ...
            L_weights['weights'] = xbar_output_red_col(bits[0],  w_inp, w_dim, red_col_map, defected_weight, all_xbr_cells)
        ...
        # Fault Handled Weights Using Both Existing Cells and Redundant Rows and Columns
            ...
            L_weights['weights'] = xbar_output_combined(bits,  w_inp, w_dim, cell_mapping, defected_weight, all_xbr_cells)
            ...
            # print("bits Prop\n{}".format(type(bits[0])))
            L_weights['weights'] = xbar_output_prop(bits[0], w_inp, w_dim, grouped_cells, f_grp_mapping, filter_size, endpoint)
            # print("L_weights Proposed\n{}".format(L_weights))
            # print("defected_weight\n{}".format(defected_weight['xbar'].shape))
            print(L_weights)
        print(L_weights)
        W_weights = L_weights['weights']
        w = W_weights[:w_inp, :].reshape((l_weight.shape))
        b = W_weights[w_inp, :].reshape(l_bias.shape)
        # print(w.shape)
        # print(b.shape)
...
def accuracy_check(converted_weight, perc_def, model, new_weight_filename='Memristor_weights.npy', re_train=False):
    ...
    #Model Accuracies
    accuracy_test, acc_retrained = model(weight_checker, new_weight_filename, tool, re_train=False)
    
    print(f'Defect percentage: {perc_def} --> Accuracy:-{tool}: {accuracy_test}')
    
    if re_train ==True:
        accuracy_test, acc_retrained = model(weight_checker, new_weight_filename, tool, re_train)
        print(f'Retrained Accuracy:-{tool}: {acc_retrained}')
...
def main(Ron, Roff, num_bits, default_weights, model, percentage_defect, retrain, approach, filter_size, fix_model,test_case, my_seed):
    ...
    # print(Ron, Roff, num_bits, default_weights, model, percentage_defect, retrain, approach, filter_size, fix_model,test_case, my_seed)
    # Considering the bit levels
...
#     default_weights = 'inception_cifar10.npy'
    ...
    # for percentage_defect in range(1, 2):
...
#           print(converted_weights)
    acc = accuracy_check(converted_weights, percentage_defect, model, new_weight_filename='Memristor_weights.npy', re_train = re_train)
    accs.append(acc[0])
    ...
    return software_based_accuracy,accs, acc_retrain
